...
class TablePaginationActions extends PureComponent {
    ...
    handleFirstPageButtonClick = event => {
        const {onChangePage} = this.state;
        onChangePage && onChangePage(event, 0);
    };

    handleBackButtonClick = event => {
        const {onChangePage, page} = this.state;
    ...
    handleLastPageButtonClick = event => {
        const {onChangePage, count, rowsPerPage} = this.state;
        onChangePage && onChangePage(event, Math.max(0, Math.ceil(count / rowsPerPage) - 1));
    };
